...
// comment created by me - validateRegisterBodyForMember
const {
  validateCallbackBody,
  validateRegisterBody,
  validateRegisterBodyForMember,
...
module.exports = {
  async callback(ctx) {
    ...
    // comment Edited by me to get whole userInfo including role info
      const userWithRoleInfo = await getService('user').fetchAuthenticatedUser(user.id);
      return ctx.send({
        jwt: getService('jwt').issue({ id: user.id }),
        user: await sanitizeUser(userWithRoleInfo, ctx),
      });
    }
    // endcomment
...
// new method created by me to allow managers to create user without email and password.
  async registerMember(ctx) {
    ...
    // comment Edited by me to override default role
    const alwaysAllowedKeys = ['username', 'role', 'isApprovedByAdmin'];
    // endcomment
    const userModel = strapi.contentTypes['plugin::users-permissions.user'];
    const modifiedAttributes = {};
    const unnecessaryFields = ['password'];

    Object.keys(userModel.attributes).forEach(key => {
      if (!unnecessaryFields.includes(key)) {
        modifiedAttributes[key] = userModel.attributes[key];
      }
    });
    const modifiedUserModel = { ... userModel, attributes: modifiedAttributes };
    const { attributes } = modifiedUserModel;
    ...
    await validateRegisterBodyForMember(params);

    // Deleted by me to override default role
    // const oldrole = await strapi
    //   .query('plugin::users-permissions.role')
    //   .findOne({ where: { type: settings.default_role } });

    // if (!role) {
    //   throw new ApplicationError('Role is not selected.');
    // }

    // comment Edited by me to override default role
    const { email, username, provider, role, isApprovedByAdmin } = params;
...
// comment by me. Change default ctx sanitizeUser to newUser, as I need to populate all fields with corresponding data
    const newUserWithID = {...newUser, id: user.id};
    return ctx.send({
      jwt,
      user: newUserWithID,
    });
  ...
  async register(ctx) {
    ...
    const { register } = strapi.config.get('plugin.users-permissions');
    // comment Edited by me to override default role
    const alwaysAllowedKeys = ['username', 'password', 'email', 'role', 'isApprovedByAdmin'];
    // endcomment
    const userModel = strapi.contentTypes['plugin::users-permissions.user'];
