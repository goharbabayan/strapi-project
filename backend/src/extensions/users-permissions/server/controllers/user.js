...
module.exports = {
   ...
   * @return {Object|Array}
   */
   async findUsersByIds(ctx) {
    const { ids } = ctx.query;
    const idsArray = ids.split(",").filter((id) => id !== "").map(Number);
    if (!ids || !Array.isArray(idsArray)) {
      return ctx.badRequest('Missing or invalid IDs');
    };

    try {
      const allUsers = await getService('user').fetchAll({
        where: { id: { in: idsArray } },
        populate: '*' // Populate related fields if needed
      });

      const filteredUsers = allUsers.filter(user => idsArray.includes(user.id));
      ctx.send(filteredUsers);
    } catch (error) {
      ctx.send(error);
    }
  },
